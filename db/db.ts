...
export async function createLinkInCategory(from: number, to: number) {
  await taskTable.add({
    name: `NOTATASKBUTALINK${to}`,
    status: false,
    parent: from,
  });
}

export async function deleteLinkAsTask(id: number) {
  await taskTable.where("id").equals(id).delete();
}
...
export async function setOrder(val: string) {
  await localStorage.setItem("order", val);
}

export async function getOrder() {
  const val = await localStorage.getItem("order");
  if (val) {
    return val;
  } else {
    setOrder("creation");
